...
func play_sound_at(sound: AudioStream, pos: Vector3, db: float = 0.0, pitch: float = 1.0) -> void:
...
func play_uid_sound_at(uid_sound: String, pos: Vector3, db: float = 0.0, pitch: float = 1.0) -> void:
	var audio_player: RaytracedAudioPlayer3D = RaytracedAudioPlayer3D.new()
	add_child(audio_player)
